...
class AllowList:
    ...
    def check(self, code: str) -> None:
        ...
        # AST analysis: defense-in-depth against semantic and reflective bypasses.
        try:
            tree = ast.parse(code)
            for node in ast.walk(tree):
                ...
                # Object introspection can recover classes, globals, builtins,
                # importers, and code objects without spelling __builtins__.
                ...
                # getattr()/hasattr() permit dynamic attribute names, so checking
                # only literal forbidden names is insufficient. Reject reflection
                # in policy-controlled profiles.
                if (
                ...
                if isinstance(node, ast.Subscript) and isinstance(node.slice, ast.Constant) and isinstance(node.slice.value, str):
                    if (
                        node.slice.value in self.blocked_builtins
            ...
            # Non-python syntax (e.g. IPython magics/shell escapes) is handled by regex rules.
            pass
    ...
    def read_only(cls) -> "AllowList":
        return cls(
            allowed_modules=[
                "pandas", "numpy", "matplotlib", "IPython", "json", "re", "math", "datetime",
                "collections", "typing",
            ],
            blocked_patterns=[
                (r'\bopen\s*\(', "file_open"),
